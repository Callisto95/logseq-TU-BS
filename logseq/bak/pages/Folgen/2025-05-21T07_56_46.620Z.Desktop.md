...
- unter einer Folge verstehen wir eine Abbildung $a:\mathbb{N}\rightarrow M$, wobei M eine beliebige Menge ist
  collapsed:: true
	- $a_{k}loneqqa\left(k\right)\in M$ heißen Gleider der Folge
	- Notation: $\left(a_{k}\right)loneqq\left(a_{k}\right)_{k\in\mathbb{N}}loneqq\left(a_{k}\right)_{k\in a}^{\infty}loneqqa$
		- ! $a_{k}\neq\left(a_{k}\right)$
	...
	- Beispiele für Folgen
		...
		- $\left(b_{k}\right)\in\mathbb{R}^{\mathbb{N}},b_{k}loneqq\frac{1}{k}$ also $\left(b_{k}\right)=\left(1,\frac12,\frac13,...\right)$
		  logseq.order-list-type:: number
...
- **rekursiv definierte Folgen**
  collapsed:: true
	- Beispiel: Bevölkerungsmodell
		...
		- Wachstumsfaktor: $w>0$
		- Limitierung: $L>0$
		- mit $b_1=a$, dann $\forall k\in\mathbb{N}:b_{k+1}loneqqb_{k}+w\left(L-b_{k}\right)b_{k}$
-
- **geometrische Summen**
	- Sei $q\in\mathbb{R}$. Definiere $\left(a_{n}\right)_{n=0}^{\infty}\subseteq\mathbb{R}$ durch $a_0=1,a_{n}loneqqa_{n-1}+q^{n}$
	...
	- Beispiel: Fibonacci-Zahlen
	  collapsed:: true
		- |Generation|0|1|2|3|4|5|...|
		  ...
		  |--|--|--|--|--|--|--|--|
		  |Zuwachs|0|0|1|1|2|3|...|
		- $a_0loneqq1,a_1loneqq1$
		- $a_{n+2}loneqqa_{n}+a_{n+1}$
	...
	- Beispiel: Dekadische Entwicklung
	  collapsed:: true
		...
		- $xloneqq\lfloor x\rfloor=\max\left\lbrace\xi\in\mathbb{Z}:\xi\leq x\right\rbrace$
		- $z_{n+1}loneqq\lfloor10^{n+1}\left(x-x_{n}\right)\rfloor$
		-
-